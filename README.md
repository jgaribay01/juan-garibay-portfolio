...
| the static hero in `evidence.html` | the browser, before JS runs | briefly |
...
## Three pages, two rules

`index.html` (the case pack) and `flight.html` (the flight) state **no** measured figure: they
derive every one in the browser from `evidence.json`, and `check.mjs` fails the build if a measured
number is ever typed into their source. `evidence.html` takes the other route — it repeats figures
so the page does not shift on load — and the guard catches drift there instead.

## Layout

```
index.html        the case pack — a shell with empty mount points and the JSON-LD
js/cases.js       the twelve-block prose; figures appear as tokens, never as numbers
js/dossier.js     fills those tokens from evidence.json and js/data.js in the browser
css/dossier.css   the pack's layout, on top of the shared tokens
flight.html       the scroll-driven WebGL corridor that used to be the front page
js/world.js       the flight's renderer; js/content.js is its prose
evidence.html     the written portfolio, and the fallback both other pages point at
js/data.js        every figure on the site, and the only place any of them is authored
js/render.js      builds evidence.html from that data — createElement and textContent only
evidence.json     the measured figures, regenerated by measure.mjs from git and disk
css/tokens.css    palette, type scale, component classes — shared by all three pages
css/page.css      evidence.html layout, accessibility corrections, the print stylesheet
check.mjs         fails if any copy of the data drifts from js/data.js
og-source.html    source for img/og.png, rebuilt by tools/render-og.sh
tools/render-og.sh  rebuilds the social card; bump the ?v= suffix after it runs
```
